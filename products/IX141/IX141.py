...
def main():
    ...
    # Random dữ liệu vào F–K dòng 11–15
    for row in range(11, 16):
        values = [
            round(random.randint(3605, 4105) / 100, 2),  # F
            round(random.randint(7205, 8105) / 100, 2),  # G
            random.randint(2620, 2810),                 # H
            random.randint(1400, 1520),                 # I
            round(random.randint(2210, 2810) / 100, 2), # J
            round(random.randint(3005, 3305) / 100, 2), # K
        ]
        for i, col in enumerate(["F", "G", "H", "I", "J", "K"]):
            cell = f"{col}{row}"
            ws.range(cell).value = values[i]
            apply_center(ws, cell)

    # Ghi PO vào F4:I4 (chỉ F4 là đủ)
